...
def main(
    ...
    error_CS2 = np.load('/home/erb/masterproject/MSCI_project/snow_ice/CS_IS_2p_LARM/error_map/2018-2019/CS2_Period_15j.npy')
    error_IS2 = np.load('/home/erb/masterproject/MSCI_project/snow_ice/CS_IS_2p_LARM/error_map/2018-2019/IS2_Period_15j.npy')
        ...
        for i in range (360):
            for j in range (360):
                if not np.isnan(is2[k][i][j]):
                    if error_IS2[i][j] > 0:
                        is2_observations.append([grid_x[i][j]/1000000, grid_y[i][j]/1000000, 1, is2[k][i][j], error_IS2[i][j], month])
                    else : 
                        is2_observations.append([grid_x[i][j]/1000000, grid_y[i][j]/1000000, 1, is2[k][i][j], default_error, month])

                if not np.isnan(cs2[k][i][j]):
                    if error_CS2[i][j] > 0:
                        cs2_observations.append([grid_x[i][j]/1000000, grid_y[i][j]/1000000, 0, cs2[k][i][j], error_CS2[i][j], month])
                    else :                         
    ...
    file_snow = "images_2/" + date[7] + ".npy" + "_snow"
    file_ice = "images_2/" + date[7] + ".npy" + "_ice"
...
#date = ["20181115", "20181215", "20190115", "20190215", "20190315", "20190415"]
