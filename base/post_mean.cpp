...
static char short_options[] = "i:I:x:X:y:Y:A:T:o:m:M:D:V:e:E:g:b:z:Z:W:H:t:s:h";
...
int main(int argc, char *argv[])
{
  ...
  double maxmodelv = 0.0, minmodelv = 0.0;
  ...
  while (status > 0) {
	...
	for (auto &m : models) {
	  m->recompute();
	  for (int j = 0; j < m->nvaluecells(); j ++) {
	    double mv = m->value_at_index(m->selectvaluecell(j));
	    if (mv > maxmodelv) {
	      maxmodelv = mv;
	    }
	    if (mv < minmodelv) {
	...
	for (int j = 0; j < latsamples; j ++) {
	  ...
	  for (int i = 0; i < lonsamples; i ++) {
	    ...
	    if (image[j * lonsamples + i] < tminv) {
	      tminv = image[j * lonsamples + i];
	    }
  ...
  if (variance_file != nullptr) {
    ...
    if ((step % 100000) == 0) {
      printf("%d (%10.6f %10.6f) (%10.6f %10.6f)\n", step, minv, maxv, minmodelv, maxmodelv);
    }
