...
int main(int argc, char *argv[])
{
  ...
  if (variance_file != nullptr) {
    for (int mi = 0; mi < nmodels; mi ++) {
      mkmodelrankpath(mi, iter, output, "inter.txt", filename);
      models[mi]->save(filename);
    }
  }
  ...
  /*if(iter % 1000 == 0){
  if (variance_file != nullptr) {
      if (saveimage(path, image, lonsamples, latsamples) < 0) {
	...
	// Update best model
	//
	double flikelihood = likelihood + norm;
	if (best_likelihood == 0.0 || flikelihood < best_likelihood) {
	  best_likelihood = flikelihood;
	  for (int i = 0; i < imagesize; i ++) {
	    best[i] = image[i];
	  }
	}
  ...
  if (best_file != nullptr) {
    ...
    local_score.val = best_likelihood;
    local_score.rank = mpi_rank;

    MPI_Reduce(&local_score, &global_score, 1, MPI_DOUBLE_INT, MPI_MAXLOC, 0, MPI_COMM_WORLD);

    MPI_Bcast(&global_score.rank, 1, MPI_INT, 0, MPI_COMM_WORLD);

    if (global_score.rank == mpi_rank) {

      if (saveimage(best_file, best, lonsamples, latsamples) < 0) {
	fprintf(stderr, "error: failed to save best image\n");
	return -1;
      }
    }
