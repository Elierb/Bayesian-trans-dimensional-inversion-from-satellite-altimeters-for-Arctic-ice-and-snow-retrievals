...
extern "C" {
  #include "/home/erb/masterproject/MSCI_project/lib/slog.h"
};
...
public:
  ...
  void initialize(int initialcells, std::vector<const char *> &initial_models)
  {
	...
	std::string model_type[4]={".A", ".B", ".C", ".D"};

	std::string path_file;

	path_file = "/home/erb/masterproject/MSCI_project/snow_ice/CS_IS_2p/results/finalmodel.txt" + model_type[mi] + "-0000000000";

    std::ifstream inputFile(path_file);
	std::string dummyLine;
    getline(inputFile, dummyLine);

	//std::cout << initialcells << std::endl;
	std::cout << path_file << std::endl;

	for (int j = 7; j < initialcells - 10; j++) {
    double xyz[3];
    std::string line;
    while (getline(inputFile, line)) {
        //std::string delimiter = " ";
        std::istringstream iss(line); // Utilisez un stringstream pour lire les tokens plus facilement
        
        // Lecture des trois valeurs dans xyz[0], xyz[1], xyz[2]
        ...
        models[mi]->add_cell(xyz[0], xyz[1], xyz[2]); // Ajoute la cellule au modèle
    }
}	} else {
